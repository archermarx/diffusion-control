...
class Surrogate:
    ...
    def suggest_next_control(self) -> np.ndarray:
        """Finds the next best control point by maximizing Expected Improvement (EI) mathematically."""
        from scipy.optimize import minimize
        
        # We need the current best physical metric to calculate Expected Improvement
        f_min = np.min(self.Y_data)
        
        def negative_EI(x_test):
            """Calculates the negative EI because Scipy only minimizes."""
            x_test_reshaped = np.array(x_test).reshape(1, -1)
            pred = self.active_model.predict_values(x_test_reshaped)
            var = self.active_model.predict_variances(x_test_reshaped)
        ...
        # Give the optimizer a starting guess (the best physical point we've seen so far)
        best_idx = np.argmin(self.Y_data)
        x_start = self.X_data[best_idx]
        
        # Define physical constraints/bounds for the optimizer
        bounds = [(self.x_lims[0], self.x_lims[1]), (self.y_lims[0], self.y_lims[1])]
        
        # Let Scipy run thousands of micro-evaluations on the SURROGATE's EI equation
        res = minimize(negative_EI, x0=x_start, bounds=bounds, method='L-BFGS-B')
        
        # Return the exact coordinates it suggests we test on the thruster next
        return res.x
...
if __name__ == "__main__":
    ...
    surrogate = Surrogate(dim=ndim, fun=problem, x_lims=[-2, 2])
    ...
    x_opt = surrogate.suggest_next_control()
